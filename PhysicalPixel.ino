/*
  ...
  this case, the Arduino boards turns on an attached light when it receives the character
  'N', and turns off the LED when it receives the character 'F'.

  The data can be sent from the Arduino Serial Monitor, or another program like
  Processing (see code below), Flash (via a serial-net proxy), PD, or Max/MSP.

  The circuit:
  - Relay connected to digital pin 7 to ground with light wired to relay
...
const int relay = 7; // the pin that the LED is attached to
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // if it's a capital N (ASCII 72), turn on the LED:
    if (incomingByte == 'N') {
...
/* Processing code for this example
  ...
  void draw() {
    ...
    if (mouseX > boxX - boxSize && mouseX < boxX + boxSize &&
      ...
      stroke(255);
      fill(153);
      // send an 'N' to indicate mouse is over square:
      port.write('N');
    }
    else {
